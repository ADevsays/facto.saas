<script setup lang="ts">
import { useBetaForm } from '../composables/useBetaForm';

const { canProceed, currentStep, totalSteps, next, back } = useBetaForm();
</script>

<template>
    <div
        class="flex justify-between items-center px-8 py-5 shrink-0"
        style="border-top: 1px solid rgba(255,255,255,0.06);"
    >
        <button
            @click="back"
            class="text-gray-500 hover:text-white text-sm transition-colors flex items-center gap-2"
            :class="currentStep === 1 ? 'invisible' : ''"
        >
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
            </svg>
            <span class="hidden sm:inline">Atrás</span>
        </button>

        <button
            @click="next"
            :disabled="!canProceed"
            class="flex items-center gap-2 px-5 py-3 rounded-full text-[11px] font-bold uppercase tracking-widest transition-all duration-300 whitespace-nowrap"
            :class="canProceed ? 'text-white cursor-pointer' : 'text-gray-600 cursor-not-allowed'"
            :style="canProceed
                ? 'background: rgba(0,212,255,0.12); border: 1px solid rgba(0,212,255,0.5); box-shadow: 0 0 20px rgba(0,212,255,0.1);'
                : 'background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08);'"
        >
            <span>{{ currentStep === totalSteps ? 'Solicitar acceso' : 'Continuar' }}</span>
            <svg class="w-3 h-3 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
            </svg>
        </button>
    </div>
</template>
