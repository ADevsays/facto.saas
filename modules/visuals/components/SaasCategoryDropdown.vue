...
<template>
  <div class="flex items-center gap-2 relative w-full sm:w-auto">
    ...
    <div class="relative w-full sm:w-auto">
      <button 
        @click="$emit('toggle')" 
        class="flex items-center justify-between gap-3 bg-white/[0.04] border border-white/10 hover:border-white/20 text-white rounded-xl px-4 py-2.5 text-xs font-sans tracking-wide transition-all duration-300 cursor-pointer min-w-[170px]"
      >
        <span class="truncate max-w-[120px] sm:max-w-none">{{ currentLabel }}</span>
      ...
      <div 
        v-if="isOpen" 
        class="absolute left-0 mt-2 w-full bg-[#0c0c10]/95 border border-white/10 rounded-xl overflow-hidden shadow-2xl backdrop-blur-md z-40 flex flex-col py-1 max-h-60 overflow-y-auto no-scrollbar animate-fade-in"
      >
        <button 
